...
class MatchupController {
  ...
  async collect({ request, response }) {
    ...
    try {
      ...
      const serverToCrawl = crawlHelpers.getNextServerNameToCrawl(config.last_craweled_server_name)
      console.log(serverToCrawl)
      // const res = await Axios.get(
      //   `https://${serverToCrawl}.api.riotgames.com/lol/match/v4/matches/${config.gameId}`,
      //   {
      //     headers: {
